...
import type { ApiKeyItem, CustomRoleItem, SystemRole } from "@/modules/platform-admin/types/platform-admin.types";
...
export default function UpdateApiKeyPage() {
  ...
  const goBack = () => router.push(`${RouteConfig.PLATFORM_ADMIN.ADMINISTRATOR.API_KEY.LIST}?highlight=${id}`);

  useEffect(() => {
    Promise.all([
      platformAdminApi.getApiKeyById(id),
      ...
      .then(([keyRes, rolesRes, sysRolesRes]) => {
        ...
        setApiKey(k);
        setKeyDescription(k.keyDescription ?? "");
        ...
        const currentList = k.roles?.length ? k.roles : (k.rolesList ?? "").split(",").map((r) => r.trim()).filter(Boolean);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await platformAdminApi.updateApiKeyById(id, {
      ...
      toast.error(err instanceof Error ? err.message : t.apiKeys.failedToUpdate);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">{t.apiKeys.editTitle}</h1>
          <p className="text-sm text-gray-500 mt-0.5">{t.apiKeys.editSubtitle} &quot;{apiKey?.keyName || id.slice(0, 8)}&quot;</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 w-full">
          <div className="bg-white border rounded-lg p-6 flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-4">
              <Input label={t.apiKeys.fieldKeyName} value={apiKey?.keyName ?? ""} readOnly disabled />
              <Input
                label={t.apiKeys.fieldDescription}
