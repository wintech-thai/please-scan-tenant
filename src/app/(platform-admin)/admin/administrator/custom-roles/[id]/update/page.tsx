...
export default function UpdateCustomRolePage() {
  ...
  const [origRoleName, setOrigRoleName] = useState("");
  ...
  const goBack = () => router.push(`${RouteConfig.PLATFORM_ADMIN.ADMINISTRATOR.CUSTOM_ROLE.LIST}?highlight=${id}`);

  useEffect(() => {
    Promise.all([platformAdminApi.getCustomRoleById(id), platformAdminApi.getInitialUserRolePermissions()])
      .then(([roleRes, permsRes]) => {
        const r = roleRes.data?.customRole;
        if (!r) throw new Error("Role not found");
        setRoleName(r.roleName);
        setOrigRoleName(r.roleName);
        setRoleDescription(r.roleDescription ?? "");
        setTags(r.tags ? r.tags.split(",").map((t) => t.trim()).filter(Boolean) : []);
        setPermGroups(r.permissions?.length ? r.permissions : permsRes.data?.permissions ?? []);
      })
      .catch(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await platformAdminApi.updateCustomRoleById(id, {
      ...
      toast.error(err instanceof Error ? err.message : t.customRoles.failedToUpdate);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">{t.customRoles.editTitle}</h1>
          <p className="text-sm text-gray-500 mt-0.5">{t.customRoles.editSubtitle} &quot;{origRoleName}&quot;</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="flex flex-col gap-6 w-full">
          <div className="bg-white border rounded-lg p-6 flex flex-col gap-4">
            <Input label={t.customRoles.fieldRoleName} isRequired value={roleName} onChange={(e) => setRoleName(e.target.value)} />
            <Input label={t.customRoles.fieldDescription} value={roleDescription} onChange={(e) => setRoleDescription(e.target.value)} />
